...
class TestDBInfoSecretsManager(unittest.TestCase):
    def test_db_info_secrets_manager(self,
        ...
        mock_db_info_from_secrets_manager.assert_called_with('secrets manager entry name')
        self.assertEqual(expected_result, db_facts)
        mock_pull_jinja_context.\
            assert_called_with(['fromage'],
                               mock_dbcli_config['dbs']['fromage'],
                               mock_dbcli_config)

    def test_db_info_pull_secrets_manager_user_and_pass_only(self,
                                                      mock_pull_aws_secrets_manager_secret,
                                                      mock_db_info_from_secrets_manager,
                                                      mock_pull_jinja_context):
        ...
        mock_pull_aws_secrets_manager_secret.assert_called_with('secrets manager entry name')
        self.assertEqual(expected_result, db_facts)
        mock_pull_jinja_context.\
            assert_called_with(['fronk'],
                               mock_dbcli_config['dbs']['fronk'],
                               mock_dbcli_config)
